...
```
{
  ...
  "random_act": [
    ...
    # 除了这些常规动作，还有5个内部使用的特殊动作名称
    # 请不要占用这些名称，这些动画也不会在动作列表中显示
    # 可以注意到，这些动作的 act_type 是 [0, 10000]，我用此来表示这个是特殊动作
    # feed_1,2,3 是吃到不同喜好食物后的反应，定义 "sound" 还可以播放语音 (语音在后面的通知个性化部分讲解)
    # check_item 是查看收藏品时播放的特殊动作，可以设置收藏品图片的显示大小与位置
    ...
    {"name":"check_item", "act_list":["checkitem_start", "checkitem", "checkitem_end"], "act_prob":0, "act_type":[0,10000],
     "item_size":[32,32], "item_anchor":[-75,-180]},
...
#### `check_item` — 查看收藏品动作
`check_item` 是查看收藏品时使用的特殊动作。用户对【收藏类】物品执行【使用】操作时，宠物会播放该动作组，并在动作期间显示对应的收藏品图片。

该动作需要定义在角色的 `pet_conf.json` → `random_act` 中，且 `name` 必须固定为 `check_item`：

```json
{
  "name": "check_item",
  "act_list": ["checkitem_start", "checkitem", "checkitem_end"],
  "act_prob": 0,
  "act_type": [0, 10000],
  "item_size": [32, 32],
  "item_anchor": [-75, -180]
...
| 参数 | 类型 | 说明 |
|:-----|:-----|:-----|
| `name` | str | 必须填写为 `check_item`，用于让程序识别该特殊动作 |
| `act_list` | list[str] | 查看收藏品时依次播放的动作列表；示例分别使用开始、展示和结束动作 |
| `act_prob` | float | 应设为 `0`，避免该特殊动作作为普通随机动作自动播放 |
| `act_type` | list[int] | 应设为 `[0, 10000]`，用于标记内部调用的特殊动作 |
| `item_size` | list[int] | 收藏品图片的显示尺寸，格式为 `[宽度, 高度]`，单位为像素 |
| `item_anchor` | list[int] | 收藏品图片相对宠物画布底边中心的偏移量，格式为 `[水平偏移, 垂直偏移]`，单位为像素。水平偏移为正时向右、为负时向左；垂直偏移为正时向下、为负时向上 |

`act_list` 中使用的每个动作仍需在角色的 `act_conf.json` 中定义，并在角色的 `action` 文件夹内提供对应的动画帧。`item_size` 和 `item_anchor` 应根据角色画布大小、动作姿势及收藏品预期出现的位置进行调整。

开发者不需要手动计算或配置收藏品图片的显示时长。软件会根据 `act_conf.json` 中各段动作的帧数、播放次数和单帧时长，自动计算收藏品应当出现及消失的时间。

建议将 `check_item` 设计为三段式动作：

1. **开始动作**：角色抬手或进入查看姿势，此阶段不显示收藏品；
2. **展示动作**：角色保持查看姿势，收藏品会在这一阶段显示；
3. **结束动作**：收藏品消失，角色收手并恢复原有姿势。

当 `act_list` 包含至少三段动作时，软件会将第一段视为开始动作、最后一段视为结束动作，并仅在中间的一段或多段动作期间显示收藏品。如果没有采用三段式结构，收藏品将会在整个 `check_item` 动作播放期间显示。
...
### 昼夜作息系统
呆啵宠物支持为角色定义一套**昼夜作息**，让桌宠在白天和夜晚切换不同的动画池，使角色更有生命感。

- **白天**：维持现有的随机动画行为
- **夜晚**：仅播放夜间动画（睡觉循环 + 偶发的夜间随机动作），暂停大部分系统事件
- **过渡动画**：入睡、起床、被打扰时的一次性过渡动画（可选）

> **昼夜系统是可选功能**。角色配置中无 `day_night` 块时，角色始终处于白天模式，行为与现有版本完全相同。用户还需在设置中手动开启昼夜作息功能后才会生效。

#### 配置方法

昼夜系统的配置分为两步：

1. 在 `random_act` / `accessory_act` 中为夜间动画添加 `"phase": "night"` 标记
2. 在 `pet_conf.json` 中添加 `day_night` 引用块

**步骤一：标记夜间动画**

在 `random_act` 或 `accessory_act` 中，为所有夜间专用动画添加 `"phase": "night"` 字段：

```json
"random_act": [
  {"name": "站立",   "act_list": ["stand"],     "act_prob": 1.0, "act_type": [2, 0]},
  {"name": "睡觉",   "act_list": ["sleep"],     "act_prob": 1.0, "act_type": [0, 0], "phase": "night"},
  {"name": "翻身",   "act_list": ["rollover"],  "act_prob": 0.3, "act_type": [0, 0], "phase": "night"},
  {"name": "上床",   "act_list": ["go_to_bed"], "act_prob": 1.0, "act_type": [0, 0], "phase": "night"},
  {"name": "起床",   "act_list": ["wake_up"],   "act_prob": 1.0, "act_type": [0, 0], "phase": "night"},
  {"name": "被吵醒", "act_list": ["disturbed"], "act_prob": 1.0, "act_type": [0, 0], "phase": "night"}
]
```

标记了 `"phase": "night"` 的动画：
- **不会出现在白天的随机播放池中**（权重强制为 0）
- 只能通过 `day_night` 块在夜间触发
- 在动作列表菜单中仍然可以手动选择

> `act_prob` 在夜间动画中作为**夜间权重**使用（多个夜间动画之间的相对概率），与白天动画的概率计算相互独立。

**步骤二：添加 `day_night` 引用块**

在 `pet_conf.json` 中添加 `day_night` 块，引用上面已定义好的动画名称：
...
| 字段 | 类型 | 是否必填 | 说明 |
|:---|:---|:---|:---|
| `sleep` | `str` 或 `list[str]` | **必填** | 基础睡眠循环动画名称（一个或多个）。多个时按各自 `act_prob` 加权随机选取 |
| `night_random_act` | `list[str]` | 选填 | 夜间低概率随机动画（如翻身、梦呓）。为空时夜间只播放睡眠动画 |
| `to_night` | `str` 或 `list[str]` | 选填 | 入睡过渡动画，在白天→夜晚边界触发一次。缺少时直接硬切到睡眠动画 |
| `to_day` | `str` 或 `list[str]` | 选填 | 起床过渡动画，在夜晚→白天边界触发一次。缺少时直接硬切到白天动画池 |
| `disturbed` | `str` 或 `list[str]` | 选填 | 夜间被用户拍拍/强行聊天时触发的反应动画，不会唤醒宠物。缺少时拍拍不产生动画反应 |
| `night_random_prob` | `float` 0–1 | 选填 | 夜间随机动画的概率，默认 `0.1` |

> 过渡动画字段（`to_night`、`to_day`、`disturbed`）支持填入列表，程序会**随机选取**其中一个播放，适合制作多样化的过渡效果。

#### 最简配置示例

只需给已有的睡觉动画加上 `phase` 标记，再写一行 `day_night` 块，即可启用昼夜系统：
...
无过渡动画，昼夜边界直接硬切，是零额外素材的最简接入方式。

#### `hide_night` — 夜间隐藏动画

当宠物隐藏到屏幕边缘且处于夜晚时，默认使用普通的 `hide` 动画。如果制作了带有睡眠感的隐藏动画，可以通过 `hide_night` 字段指定：

```json
"hide": "hide",
"hide_night": "hide_sleep"
```

`hide_night` 是可选字段，缺少时夜间隐藏动画与白天相同。

#### 关于 `act_type` 与夜间动画

夜间动画的选择**不受饱食度限制**，`act_type` 对夜间播放没有影响。建议将夜间动画的 `act_type` 设为 `[0, 0]`，以保持配置可读性。

#### 关于昼夜时间配置

昼夜边界时间（几点开始白天、几点开始夜晚）是**全局用户设置**，由用户在设置界面自行配置（默认白天 08:00、夜晚 23:00），不属于角色模组的配置范围。
...
| collection | 收藏品点击使用时，不会触发任何数值效果；因此，无需填写 ``effect_HP, effect_FV``；建议不要填写 ``drop_rate, buff``。如需让角色查看收藏品，可参考角色开发中的 [`check_item` — 查看收藏品动作](#check_item--查看收藏品动作) |
...
## 进阶功能
本章节介绍需要同时配置多个模组或涉及特殊联动的可选功能。如果不需要这些功能，可以跳过本章节，不影响普通角色、动作或物品模组的开发。

### 可穿戴物品（Equip）
Equip 是一种可以作为额外外观、持续显示在角色身上的特殊收藏品，例如翅膀、披风或背部装饰。用户在背包中使用物品后，对应组件会直接出现在角色身上；再次使用同一物品，或通过组件的右键菜单，可以将其收回。

Equip 功能需要同时完成两部分配置：

1. 在物品模组的 `items_config.json` 中定义一个 `collection` 类型的物品；
2. 在角色模组的 `pet_conf.json` → `accessory_act` 中定义一个与物品**完全同名**、且 `acc_type` 为 `equip` 的组件。

> **注意：** Equip 物品在物品模组中的 `type` 仍然是 `collection`，并不存在名为 `equip` 的物品类型。`equip` 只填写在角色 `accessory_act` 的 `acc_type` 参数中。

#### 使用限制
Equip 本质上是一个跟随角色整体位置移动的独立组件图层。它不会识别角色每一帧的身体轮廓，也不会随着不同姿势自动变形或重新定位。

因此，Equip 更适合画布尺寸、身体位置与 Anchor 基准较为一致，且动作幅度较小的角色。如果角色在不同动作中的身体位置差异较大、姿势变化剧烈，或者各套素材的构图不一致，穿戴组件很容易发生错位。制作前请先评估角色素材是否适合，并充分测试行走、睡眠、下落及特殊动作等状态。

#### 第一步：在物品模组中定义收藏品
首先，在物品模组的 `items_config.json` 中定义物品。以下为“风雷翅”的配置示例：

```json
"风雷翅": {
  "image": "23_fengleichi.png",
  "description": "蕴含风雷之力的飞行法宝，展开双翼便可施展迅疾无比的雷遁之术。",
  "type": "collection",
  "cost": -1,
  "fv_lock": 3,
  "pet_limit": ["韩立"],
...
`pet_limit` 并非 Equip 的必要参数，但角色专属外观建议通过该参数限制可使用的角色。物品的其他通用参数请参考[物品开发](#物品开发)。

#### 第二步：准备组件动画
Equip 使用的组件动画与普通 `accessory_act` 相同，需要在角色的 `act_conf.json` 中定义。以下为“风雷翅”使用的 `fengleichi` 组件动画：
...
对应的透明背景 PNG 动画帧应放在角色的 `action` 文件夹中，并按 `fengleichi_0.png`、`fengleichi_1.png`、……的方式命名。静态穿戴物也可以只准备一帧图片。

#### 第三步：在角色中定义同名 Equip 组件
在角色的 `pet_conf.json` → `accessory_act` 中加入 Equip 配置。`name` 必须与物品模组中的物品名称完全一致：
...
用户使用该收藏品时，软件会按物品名称查找 `accessory_act` 中的同名配置，并直接生成组件，不会播放角色动作。配置为持续存在的 Equip 会被软件记录，并在下次启动时自动恢复。

#### Equip 参数说明
| 参数 | 推荐值 | 说明 |
|:-----|:-------|:-----|
| `name` | 与物品名称一致 | 必须与 `items_config.json` 中的物品名称完全一致，否则使用物品时无法找到该组件 |
| `act_list` | `[]` | Equip 会直接生成组件，不需要同时播放角色动作 |
| `acc_list` | 组件动作列表 | 列出在角色 `act_conf.json` 中定义的组件动画名称 |
| `act_type` | `[0, 10000]` | 将其标记为内部调用的特殊组件，避免作为普通随机动作播放 |
| `anchor` | 按素材调整 | 组件相对角色画布底边中心的偏移量；水平正值向右、负值向左，垂直正值向下、负值向上 |
| `act_prob` | `0.0` | 避免 Equip 作为普通组件动画随机播放 |
| `acc_type` | `"equip"` | 必填，用于将该组件识别为可穿戴物品 |
| `follow_main` | `true` | 让组件跟随角色整体移动 |
| `follow_lock` | `true` | 让组件即时锁定在角色 Anchor 上，避免移动时出现延迟或拖尾 |
| `timeout` | `false` | 让组件持续存在，不在动画播放结束后自动关闭；同时允许软件记录并在下次启动时恢复 |
| `unique` | `true` | 保证同名组件只存在一个；组件已显示时再次使用该物品会将其收回 |
| `closable` | `true` | 允许用户通过组件右键菜单选择“收回” |
| `above_main` | 按图层需要设置 | `true` 显示在角色上层，`false` 显示在角色下层；翅膀等背部装饰通常设为 `false` |

#### 素材与定位建议
- 在角色的默认、行走、睡眠、下落和常用特殊动作中分别测试组件位置，不要只测试待机动作。
- 优先保证所有角色动画使用一致的画布尺寸、底边中心与身体基准位置。
- 如果组件需要贴合身体轮廓，建议为其保留足够透明边距，并通过 `anchor` 反复微调。
- `above_main` 只能决定组件位于整个角色图层的上方或下方，无法让组件的一部分在角色前方、另一部分在角色后方。
- 如果角色的姿势和身体位置变化幅度很大，建议不要使用 Equip，或将外观直接绘制进每个角色动作中，以避免明显错位。
