...
class PetConfig:
    ...
    def init_config(cls, pet_name: str, pic_dict: dict, size_factor):
        configdir = getFile().locateresourcesfolder(str='role/') + '{}/pet_conf.json'
        path = configdir.format(pet_name)
        with open(path, 'r', encoding='UTF-8') as f:
            ...
            # 
            # 初始化所有动作
            act_path = getFile().locateresourcesfolder(str='role/') + '{}/act_conf.json'.format(pet_name)
            ...
            # 对话列表
            msg_file = getFile().locateresourcesfolder(str='role/') + '{}/msg_conf.json'.format(pet_name)
    ...
    def init_sys(cls, pic_dict: dict, size_factor):
        path = getFile().locateresources(str='role/sys/sys_conf.json')
        with open(path, 'r', encoding='UTF-8') as f:
            o = PetConfig()
            conf_params = json.load(f)
            ...
            # 
            # 初始化所有动作
            act_path = getFile().locateresources(str='role/sys/act_conf.json')
            act_conf = dict(json.load(open(act_path, 'r', encoding='UTF-8')))
            act_dict = {}
...
class Act:
    ...
    def init_act(cls, conf_param, pic_dict, scale, pet_name):
        ...
        img_dir = getFile().locateresourcesfolder(str='role/') + '{}/action/{}'.format(pet_name, images)
...
class ItemData:
    ...
    def __init__(self):
        ...
        self.file_path = getFile().locateresources(str='items/items_config.json')
        self.item_dict = {}
...
def _load_item_img(img_path):
    ...
    img_file = getFile().locateresourcesfolder(str='items/') + '{}'.format(img_path)
