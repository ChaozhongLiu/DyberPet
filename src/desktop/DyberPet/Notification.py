...
class DPNote(QWidget):
    def __init__(self, parent=None):
        ...
        sys_note_conf = dict(json.load(open(getFile().locateresources(str='icons/note_icon.json'), 'r', encoding='UTF-8')))
        try:
            pet_note_conf = dict(json.load(open(getFile().locateresourcesfolder(str='role/') + '{}/note/note.json'.format(settings.petname), 'r', encoding='UTF-8')))
        except:
            pet_note_conf = {}
        ...
        pet_cof = dict(json.load(open(getFile().locateresourcesfolder(str='role/') + '{}/pet_conf.json'.format(settings.petname), 'r', encoding='UTF-8')))
    ...
    def init_note(self, sys_note_conf, pet_note_conf):
        ...
        for k, v in sys_note_conf.items():
            if k in pet_note_conf.keys():
                if 'image' in pet_note_conf[k].keys():
                    img_file = getFile().locateresourcesfolder(str='role/') + '{}/note/{}'.format(settings.petname, pet_note_conf[k]['image'])
                    #image = QImage()
                    #image.load(img_file)
                else:
                    img_file = getFile().locateresourcesfolder(str='icons/') + '{}'.format(sys_note_conf[k].get('image', 'icon.png'))
                    #image = QImage()
                    #image.load(img_file)

                if 'sound' in pet_note_conf[k].keys():
                    #player = QSoundEffect()
                    url = getFile().locateresourcesfolder(str='role/') + '{}/note/{}'.format(settings.petname, pet_note_conf[k]['sound']) #QUrl.fromLocalFile(getFile().locateresourcesfolder(str='roles/') + '{}/note/{}'.format(settings.pet_data.petname, pet_note_conf[k]['sound']))
                    #player.setSource(url)
                    #player.setVolume(0.4)
                else:
                    #player = QSoundEffect()
                    url = getFile().locateresourcesfolder(str='sounds/') + '{}'.format(sys_note_conf[k].get('sound', 'Notification.wav')) #QUrl.fromLocalFile(getFile().locateresourcesfolder(str='sounds/') + '{}'.format(sys_note_conf[k].get('sound', '13945.wav')))
                    #player.setSource(url)
                    #player.setVolume(0.4)
        ...
        for k, v in pet_note_conf.items():
            ...
            if 'image' in pet_note_conf[k].keys():
                ...
                img_file = getFile().locateresources(str='icons/icon.png')

            if 'sound' in pet_note_conf[k].keys():
                url = getFile().locateresourcesfolder(str='role/') + '{}/note/{}'.format(settings.petname, pet_note_conf[k]['sound'])
            else:
                url = getFile().locateresources(str='sounds/Notification.wav')
        ...
        #sound_file = getFile().locateresourcesfolder(str='sounds/') + '{}'.format(icon_params.get('sound', '13945.wav'))
