...
class PetWidget(QWidget):
    ...
    def __init__(self, parent=None, curr_pet_name='', pets=(), screens=[]):
        ...
        self.current_screen = settings.screens[0]
    ...
    def mouseMoveEvent(self, event):
        ...
        if Qt.LeftButton and self.is_follow_mouse:
            ...
            if settings.onfloor == 1: # and settings.mouseposx3!=0:
                if settings.set_fall == 1:
                    settings.onfloor=0
                settings.draging=1
                self.workers['Animation'].pause()
    ...
    def _init_ui(self):
        ...
        image = QImage()
        image.load(getFile().locateresources(str='icons/HP_icon.png'))
        ...
        image = QImage()
        image.load(getFile().locateresources(str='icons/Fv_icon.png'))
        ...
        image = QImage()
        image.load(getFile().locateresources(str='icons/Timer_icon.pngicons/Timer_icon.png'))
    ...
    def _set_tray(self) -> None:
        ...
        if self.tray is None:
            ...
            self.tray.setIcon(QIcon(getFile().locateresources(str='icons/icon.png')))
...
def _load_all_pic(pet_name: str) -> dict:
    ...
    :return: {动作编码: 动作图片}
    """
    img_dir = getFile().locateresourcesfolder(str='role/') + '{}/action/'.format(pet_name)
...
if __name__ == '__main__':
    # 加载所有角色, 启动应用并展示第一个角色
    pets = read_json(getFile().locateresources(str='role/pets.json'))
